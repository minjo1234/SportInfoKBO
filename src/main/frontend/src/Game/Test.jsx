...
export default function Test() {
    ...
    const [TodayGameList, setTodayGameList] = useState(() => {
        const savedData = localStorage.getItem('TodayGameList');
    ...
    const clearLocalStorage = () => {
        localStorage.removeItem('TodayGameList');
    ...
    useEffect(() => {
        async function fetchData() {
            try {
                ...
                const res = await axios.get('/CurrentGameController');
                const data = res.data;
                const parsedData = JSON.parse(data.replace(/'/g, "\""));

                // 시간을 1초씩 경과하도록 업데이트
                setTodayGameList(parsedData);
                localStorage.setItem('TodayGameList', JSON.stringify(parsedData));
                setIsLoading(false);
            } catch (err) {
                console.error(err);
                setIsLoading(false);
